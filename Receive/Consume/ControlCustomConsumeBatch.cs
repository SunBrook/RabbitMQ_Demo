...
{
    public class ControlCustomConsumeBatch
    {
        ...
        public ControlCustomConsumeBatch(int channelCount, int consumerCount, ushort batchCount, string exChangeName, string queueName, string routingKeyName, Action<string> action)
        {
        ...
        public void CreateConsumer(int count)
        {
            ...
            foreach (var connectItem in _ConnectChannels)
            {
                foreach (var channelItem in connectItem.ChannelConsumes)
                {
                    ...
                    while (addCount > 0)
                    {
                        var consumer = new AsyncEventingBasicConsumer(channelItem.Channel);
            ...
            while (count > 0)
            {
                ...
                {
                    Connection = MQInitForReceivedConsume.GetConnectionBatch(_batchCount),
                ...
                for (int i = 0; i < _channelCount; i++)
                {
                    ...
                    channelConsumeStatus.Channel.BasicQos(0, _batchCount, false);
                    // 创建一个新的，持久的交换区
                    ...
                    {
                        var consumer = new AsyncEventingBasicConsumer(channelConsumeStatus.Channel);
                        ...
                        consumer.Received += async (model, ea) =>
                        {
                            await Task.Run(() =>
                            {
                                ...
                                // 手动应答消息
                                lock (channelConsumeStatus.Channel)
                                {
